...
const DT = {
    ...
    assist: {
        condition: (mgr, state, p, cmd) => mgr.getVisible('p"Supercomputer"'),
        trueCond: "pass",
        falseCond: "wait",
    },
    wait: {
    ...
    pass: {
        exec(mgr, state, p, cmd){
            console.log(p)
            state.player_angle = mgr.getAngle('p"Supercomputer"');
            state.player_distance = mgr.getDistance('p"Supercomputer"');

...
module.exports = {
    kick(infoAnalyzer){
        ...
        for (let i = 0; i < 3; i++){
            let flag = flags[i];
            if (infoAnalyzer.state.all_flags[flag]){
                return {n: "kick", v: "100 " + (infoAnalyzer.state.all_flags[flag].angle + plus[i])};
            }
        }

        let random_flag = infoAnalyzer.state.all_flags[Object.keys(infoAnalyzer.state.all_flags)[0]];
        if (side == "l"){
            if (random_flag.y > 0){
    ...
    seeDir(infoAnalyzer){
        ...
        let side = infoAnalyzer.side;
        let sign = (side == 'l') ? 1 : -1;
        let keys = Object.keys(infoAnalyzer.state.all_flags);
        for (const key of keys){
            let flag = infoAnalyzer.state.all_flags[key];
            if (Math.abs(flag.x) < 50){
                continue;
            }
            let dir = (flag.x > infoAnalyzer.state.pos.x) ? 1 : -1;
            if (dir == sign){
                return true;
            }
        }
    ...
    pass(infoAnalyzer){
        ...
        for (const player of infoAnalyzer.state['myTeam']){
            ...
            */
            ...
            let stake;
            if (dir){
                stake = infoAnalyzer.fw_p;
            } else {
                stake = infoAnalyzer.back_p;
            }
    ...
    teamTaken(infoAnalyzer){
        if (!infoAnalyzer.state.ball || !infoAnalyzer.state.pos){
            return false;
        }
        if (!infoAnalyzer.state['myTeam']){
            return false;
        }
        let ball = infoAnalyzer.state.ball;
        let pos = infoAnalyzer.state.pos;
    ...
    getSpeed(x, direction, center, infoAnalyzer){
        return 100;
        let key = Object.keys(infoAnalyzer.state.all_flags)[0];
        let flag = infoAnalyzer.state.all_flags[key];
